...
- content script는 **world: MAIN** — 페이지의 `FroalaEditor.INSTANCES`에서 인스턴스를 찾아 `html.set()` + `contentChanged` 트리거로 주입해야 아임웹 미리보기·저장과 연동된다 (요약설명 에디터 = Froala 3.1.1, `.fr-element` contenteditable)
