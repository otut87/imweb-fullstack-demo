...
- 생성은 **서버 프록시 단일 경로** `POST /api/summary`(enchante-pickup `app/routers/ai.py` — 키는 서버 .env `ANTHROPIC_API_KEY`에만, IP당 시간 20회 리밋, CORS는 `chrome-extension://.*` 정규식 허용). 확장에는 키·설정 UI 없음 — 공개 리포에 키 내장은 불가(GitHub secret scanning이 자동 폐기), **저장소에 키 커밋 절대 금지**. 생성 실패 시 템플릿 폴백
