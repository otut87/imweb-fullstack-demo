...
- 생성 경로 2종 — **기본: 서버 프록시** `POST /api/summary`(enchante-pickup `app/routers/ai.py`, 키는 서버 .env `ANTHROPIC_API_KEY`에만, IP당 시간 20회 리밋, CORS는 `chrome-extension://.*` 정규식 허용) / **개인 키 모드**: 팝업이 Anthropic 직접 호출(`anthropic-dangerous-direct-browser-access` 헤더로 CORS 통과, 키는 chrome.storage.local에만 — **저장소 커밋 절대 금지**, 공개 리포의 키는 GitHub secret scanning이 자동 폐기). 실패 시 템플릿 폴백
